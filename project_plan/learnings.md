...
### 快速功能：窄屏 .project-card 与 .doc-item 高度对齐
- **类型**：架构洞察 + 重构机会
- **描述**：首页"主要开源项目"和"个人总结文档"两个 section 桌面态是对称双列，窄屏塌缩为单列后视觉严重失衡——`.project-card` 带 `aspect-ratio: 1/1.1` 变成全宽大方块，`.doc-item` 则是内容自适应的紧凑横条。本次用窄屏 @media 把 `.project-card` 改为 row + 小号排版对齐 `.doc-item`。这揭示出首页"卡片"概念散落在 `.project-card`、`.doc-item`、`.portfolio-home-card`、`.about-card`、`.proj` 等多个样式里，每个都有各自的塌缩规则，难以保证一致性
- **建议处理方式**：下次做首页风格调整时考虑抽一个 `.home-card` 基类，统一 padding / radius / 塌缩行为；各 section 只覆盖差异项（aspect-ratio、内容排版）。当前改动是局部对齐，不是根治
- **紧急程度**：低
...
### 快速功能：管理后台版本控制栏
把自更新 UI 从独立页重排到仪表盘的「系统版本」栏（当前/最新版本 + 检测新版本按钮 + 更新入口），横幅退化为纯提醒。6 项反思多数为无，记录两点。

- **类型**：架构洞察（可复用技巧）
- **描述**：版本栏要展示由中间件注入 request context 的更新状态，但仪表盘模板的内容区在 `{{ with .Data }}` 作用域内，`.Update` 不在 `.Data` 下。用模板根变量 `$.Update` 直接取 context 注入的兄弟数据，**无需改动 Dashboard handler**——这是「在某页面顺带渲染一处跨切面状态」的干净做法，下次类似需求（如全局通知、配额提示）可照搬。
- **建议处理方式**：无需处理，作为模式记录。
- **紧急程度**：低
...
- 2026-07-12 快速功能「正文图片宽度与文字对齐」完成，无 learnings（已执行反思清单）

## 2026-07-12

### 快速功能：添加 CI 与 Release
- **类型**：架构洞察
- **描述**：项目的 `make check` 为方便本地开发，会在未安装 `golangci-lint` 或 `govulncheck` 时跳过对应门控；CI 若只照搬 `make check`，表面成功却没有执行完整质量检查。首次启用新版 lint 又发现 12 条既有基线告警，因此 CI 显式安装固定版本工具，并用 PR base/main push 前一提交作为 `--new-from-rev`，阻止新增债务而不让历史问题卡死流水线。
- **建议处理方式**：保留本地的可选工具策略；CI 固定工具版本并执行增量 lint。后续单独清理既有 12 条告警后，再切换为全量 lint。
- **紧急程度**：低

- **类型**：Bug
- **描述**：原 `make build` 会在编译后立即执行产物检查版本，导致在 amd64 Runner 上交叉编译 arm64 时出现 `exec format error`。新增默认开启的 `VERIFY_BUILD` 开关，Release 交叉构建时关闭执行验证，正常本地构建仍保留原门控。
- **建议处理方式**：后续新增其他跨平台构建目标时同样显式设置 `VERIFY_BUILD=0`，并通过归档内容与目标架构检查验证产物。
- **紧急程度**：中

- **类型**：Bug
- **描述**：现有安装文档通过 GitHub Release 直接下载 `manage.sh`，只上传架构归档会让首次安装 URL 返回 404。Release workflow 需同时上传 `deploy/manage.sh`，并保持资产名为 `manage.sh`。
- **建议处理方式**：发布资产契约测试持续覆盖 `manage.sh`、两种架构归档及各自 SHA256。
- **紧急程度**：高

- **类型**：技术债
- **描述**：首次强制执行 `govulncheck` 发现 Go 1.26.2 标准库存在 7 条可达漏洞，Goldmark 1.7.8 存在 1 条可达 XSS（GO-2026-5320）。为保证 CI 与 Release 从启用起即为绿色且不发布已知脆弱产物，本次同步升级到 Go 1.26.5 和 Goldmark 1.7.17。
- **建议处理方式**：保留 CI 中固定版本的 `govulncheck` 全量扫描，并定期升级 Go 补丁版本与依赖。
- **紧急程度**：高（已处理）

- 2026-07-12 快速功能「自动发布信息 release.md」完成，无 learnings（已执行反思清单）
